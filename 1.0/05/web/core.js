...
var renderMessage = function(messageList, callback) {
	...
	var friendTpl = '<span class="rong-message-notify">和 {{this.content.targetUserId}} 已是好友啦！</span>';
	...
	for (var i = 0; i < messageList.length; i++) {
		...
		var isContact = (message.messageType == messageType.ContactNotificationMessage);
		if (isContact) {
			t = friendTpl;
		}
...
var showConversation = function(){
	var friends = tools.getDom('rong-friends');
	friends.style.display = 'none';

	var conversations = tools.getDom('rong-conversations');
	conversations.style.display = 'block';
};

setTimeout(function(){
	var conversationEl = tools.getDom('rong-nav-conversation-list');
	conversationEl.onclick = function(){
		showConversation();
		refreshConversations();
	};
});
...
var refreshConversations = function() {
	getConversationList(function(list) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			var isContact = (message.messageType == RongIMClient.MessageType.ContactNotificationMessage);
			if (isContact) {
				message.content.content = '[添加好友通知]';
			}
...
var Friend = {
	getList: function(user, callback){
		callback = callback || tools.noop;
		var id = user.id;
		var url = 'http://127.0.0.1:8585/friendship/all/' + id;
		tools.request({
			url: url,
			headers: {
				'Content-Type': 'application/json'
	...
	add: function(params, callback){
		callback = callback || tools.noop;
		var url = 'http://127.0.0.1:8585/friendship/add';
		tools.request({
			url: url,
			body: params,
			method: 'POST',
...
var renderFriendList = function(){
	...
	Friend.getList({
		...
		var header = [
			'<div class="rong-header rong-conversations-header">',
			'<div class="rong-friend-create" title="添加好友" id="rong-friend-create">',
			'</div>',
			'</div>'
		].join('');
		var tpl = [
			'<div class="rong-friend" target={{this.id}} username={{this.name}}>',
			'<div class="rong-avatar rong-friend-avatar" style="background-image: url({{this.portrait}});"></div>',
			'<div class="rong-friend-name">{{this.id}}</div>',
			'</div>'
		].join('');

		var html = '', friends = result.friends;
		for (var i = 0; i < friends.length; i++) {
			var friend = friends[i];
			html += tools.render(friend, tpl)
		}
		var el = tools.getDom('rong-friends');
		header += html;
		el.innerHTML = header;

		var addEvent = function() {
			var els = document.getElementsByClassName('rong-friend');
			for (var i = 0; i < els.length; i++) {
				var el = els[i];
				el.onclick = function(e) {
					var id = e.target.getAttribute('target');
					var name = e.target.getAttribute('username');
					var type = 1;
					switchConversation({
						type: type,
						target: id
			...
			var friendEl = tools.getDom('rong-friend-create');
			friendEl.onclick = function(){
				var search = location.search;
				var userId = search.replace('?', '');
				// 随机生成好友 Id
				var friendId = 'User' + Math.floor(Math.random() * 1000);
...
setTimeout(function(){
	var friend = tools.getDom('rong-nav-friend-list');
